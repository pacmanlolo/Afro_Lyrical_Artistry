...
classes = {"Song": Song, "Artiste": Artiste, "Biography": Biography,
           "Suggestion": Suggestion}


class Storage:
    """This class manages the MySQL database for Afro Lyrical Artistry"""
    __engine = None
    __session = None

    def __init__(self):
        self.__engine = create_engine('mysql+mysqldb://{}:{}@{}/{}'.format(
            os.getenv("ARTISTE_MYSQL_USER"), os.getenv("ARTISTE_MYSQL_PWD"),
            os.getenv("ARTISTE_MYSQL_HOST"), os.getenv("ARTISTE_MYSQL_DB"),
            pool_pre_ping=True))
    ...
    def all(self, cls=None):
        ...
        obj_dict = {}
        if cls is None:
            for obj in self.__session.query(Song, Artiste, Biography,
                                            Suggestion).all():
